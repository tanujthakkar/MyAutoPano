...
class HomographyDataset(Dataset):
    ...
    def __init__(self, base_path):
        ...
        if (len(self.patchA_files) != len(self.patchB_files)) and (self.gt.shape[0] != len(self.patchA_files)):
            raise ValueError('There should be equal number of patches in base path!')

    def __len__(self):
        return len(self.patchA_files)
    ...
    def __getitem__(self, index):
        patchA_img = (imread(self.patchA_files[index]) - 127.5) / 127.5
        patchB_img = (imread(self.patchB_files[index]) - 127.5) / 127.5
